...
// LoadSecurityReport reads the cached report for a repo and when it was fetched.
// A missing or corrupt cache, or one fetched for a different state, returns
// ok=false.
func LoadSecurityReport(owner, repo, state string) (report *model.SecurityReport, fetchedAt time.Time, ok bool, err error) {
	...
	return rec.Report, rec.FetchedAt, true, nil
}

// SaveSecurityReport writes the report for a repo, stamping the current time and
// the state it was fetched for, and creating directories as needed.
func SaveSecurityReport(r *model.SecurityReport) error {
	...
	rec := securityRecord{Owner: r.Owner, Repo: r.Repo, State: r.State, FetchedAt: time.Now(), Report: r}
