...
This repository doubles as a Homebrew tap. Tap it once, then install the cask —
it downloads the latest release binary for your architecture:

```sh
brew tap justanotherspy/shuck https://github.com/justanotherspy/shuck
brew install --cask shuck
```

Upgrade later with `brew upgrade --cask shuck`. The cask
([`Casks/shuck.rb`](Casks/shuck.rb)) is regenerated by the release pipeline on
every release, so it always points at the latest binaries and their checksums.
Homebrew casks are macOS-only; on Linux use the install script or `go install`
below.
